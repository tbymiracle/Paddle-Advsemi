...
import logging
logging.basicConfig(filename='advsemiseg.log',level=logging.DEBUG)
...
ITER_SIZE = 1
NUM_WORKERS = 2
DATA_DIRECTORY = '../../old/VOCdevkit/VOC2012'
...
def main():
    ...
    if args.partial_data is None:
        trainloader = DataLoader(train_dataset,
                        batch_size=args.batch_size, shuffle=True, num_workers=2, use_shared_memory=True)
        ...
        trainloader = DataLoader(train_dataset,
                        batch_sampler=train_sampler, num_workers=2, use_shared_memory=True)
        trainloader_remain = DataLoader(train_dataset,
                        batch_sampler=train_remain_sampler, num_workers=2, use_shared_memory=True)
        trainloader_gt = DataLoader(train_gt_dataset,
                        batch_sampler=train_gt_sampler, num_workers=2, use_shared_memory=True)
    ...
    for i_iter in range(args.num_steps):
        ...
        if (args.lambda_semi > 0 or args.lambda_semi_adv > 0 ) and i_iter >= args.semi_start_adv :
            try:
                ...
                _, batch = next(trainloader_remain_iter)
            # for batch_id, (image, label) in enumerate(trainloader_gt()):
            #     print(batch_id)
            # only access to img
            images, _, _, _ = batch
        ...
        if i_iter >= args.num_steps-1:
            ...
            paddle.save(optimizer.state_dict(),osp.join(args.snapshot_dir, 'VOC_opt_'+str(args.num_steps)+'.pdparams'))
            paddle.save(optimizer_D.state_dict(),osp.join(args.snapshot_dir, 'VOC_opt_'+str(args.num_steps)+'_D.pdparams'))
            break
        ...
        if i_iter % args.save_pred_every == 0 and i_iter!=0:
            ...
            paddle.save(optimizer.state_dict(),osp.join(args.snapshot_dir, 'VOC_opt_'+str(i_iter)+'.pdparams'))
            paddle.save(optimizer_D.state_dict(),osp.join(args.snapshot_dir, 'VOC_opt_'+str(i_iter)+'_D.pdparams'))

    end = timeit.default_timer()
    logger.info(end-start,'seconds')
...
if __name__ == '__main__':
    ...
    # # optimizer and lr
    # poly_lr = optim.lr.PolynomialDecay(learning_rate=args.learning_rate,
    #                                                 end_lr=0.0,
    #                                                 power=args.power,
    #                                                 decay_steps=args.num_steps)

    # optimizer = optim.Momentum(parameters=model.optim_parameters(args),
    #                            learning_rate=poly_lr, momentum=args.momentum, weight_decay=args.weight_decay)
    # optimizer.clear_grad()
