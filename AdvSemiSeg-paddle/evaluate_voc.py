...
def main():
    ...
    model.load_dict(saved_state_dict)

    model.eval()
    model.cuda(gpu0)

    VOC = VOCDataSet(args.data_dir, args.data_list, crop_size=(505, 505), mean=IMG_MEAN, scale=False, mirror=False)

    testloader = data.DataLoader(VOC,batch_size=1, shuffle=False, pin_memory=True)
