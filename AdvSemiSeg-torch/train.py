...
if __name__ == '__main__':
    ...
    # reprod_logger.add("forward_D", outputD.detach().numpy())
    ...
    pred = interp(model(input))
    reprod_logger.add("pred", pred.cpu().detach().numpy())
    pred_label = 0
    gt_label = 1
    ...
    print(loss_seg)
    reprod_logger.add("loss_seg", loss_seg.cpu().detach().numpy())

    D_out = interp(model_D(F.softmax(pred)))
    reprod_logger.add("D_out", D_out.cpu().detach().numpy())

    loss_adv_pred = bce_loss(D_out, make_D_label(gt_label, ignore_mask))

    reprod_logger.add("loss", loss_adv_pred.cpu().detach().numpy())
    print(loss_adv_pred)
