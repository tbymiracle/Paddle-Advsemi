...
def adjust_learning_rate(optimizer, i_iter):
    ...
    # print('optimizer.param_groups:',optimizer.param_groups[0])
...
def main():
    ...
    adjust_learning_rate(optimizer, 100)
    model.cuda(args.gpu)
    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            D_out = interp(model_D(F.softmax(pred)))
            reprod_logger.add("d{i}".format(i = i_iter), D_out.cpu().detach().numpy())

            loss_adv_pred = bce_loss(D_out, make_D_label(gt_label, ignore_mask))
            
            reprod_logger.add("loss_adv_predd{i}".format(i = i_iter), loss_adv_pred.cpu().detach().numpy())
...
if __name__ == '__main__':
    ...
    fake_data = np.load('../AdvSemiSeg-paddle/align/fake_data.npy', allow_pickle=True)
    fake_label = np.load('../AdvSemiSeg-paddle/align/fake_label.npy', allow_pickle=True)
    input = torch.from_numpy(fake_data).cuda()
    label = torch.from_numpy(fake_label)

    model = Res_Deeplab(num_classes=args.num_classes)
    model_D = FCDiscriminator(num_classes=args.num_classes)

    model.load_state_dict(torch.load('./model/AdvSemiSegVOC0125.pth', map_location=torch.device('cpu')))
    model_D.load_state_dict(torch.load('./model/init_Dis_torch.pth', map_location=torch.device('cpu')))
    model.cuda()
    model_D.cuda()
    model.eval()
    model_D.eval()
    ...
    for i_iter in range(5):
        ...
        reprod_logger.add("loss{i}".format(i = i_iter), loss.cpu().detach().numpy())
        reprod_logger.add("loss_D{i}".format(i = i_iter), loss_D.cpu().detach().numpy())
